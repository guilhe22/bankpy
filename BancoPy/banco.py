...
def efetuar_transferencia() -> None:
    if len(contas) > 0:
        ...
        if conta_o:
            numero_de: int = int(input('Informe o número da sua conta: '))

            conta_d: Conta = buscar_conta_por_numero(numero_de)
            ...
            if conta_d:
                ...
                print(f'A conta destino com número {numero_de} não foi econtrada.')
        else:
...
def listar_contas() -> None:
    if len(contas) > 0:
        ...
        print('Não existem contas cadrastradas!')
    sleep(2)
    menu()
